...
![连接卡在服务器复制](assets/vscode连接jms_1779161787.png)

VS Code 进行 SSH 连接时，右下角状态栏一直停留在类似 `设置 SSH 主机 10.10.3.33: 正在使用 scp 将 VS Code 服务器复制到主机` 的阶段。

**原因**：

终端自动加载 `.cshrc` 时，如果其中包含向终端输出的操作（例如图中的 `OS: ubuntu (skipping CentOS configs)`），这些输出会干扰 VS Code 对连接状态的判断，从而导致复制操作失败。`.cshrc` 通常会嵌套多层 csh 脚本，逐一去除所有终端输出较为繁琐。

**解决方案**：

- 临时将家目录下的 `.cshrc` 全部注释。

> **潜在影响**：CentOS 开发环境可能依赖 `.cshrc` 中的配置，全部注释后可能影响正常使用。建议仅在排查问题时临时操作，完成后恢复原配置。
---
