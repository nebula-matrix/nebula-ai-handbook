...
日常工程任务中使用 `10.10.5.6` 等跳转机为 CentOS 发行版，而 `10.10.5.7` 为 Ubuntu 发行版，两者家目录下的 `.cshrc` 配置可能存在冲突。

因此建议在首次连接前检查并处理 `.cshrc`，否则 VS Code 可能卡在服务器复制阶段：

> **原因**：终端自动加载 `.cshrc` 时，如果其中包含向终端输出的操作（例如下图所示的 `OS: ubuntu (skipping CentOS configs)`），这些输出会干扰 VS Code 对连接状态的判断，从而导致复制操作失败。
>
> ![VS Code 连接卡在服务器复制阶段](assets/vscode连接jms_1779161787.png)

**解决方法**：

- 临时将家目录下的 `.cshrc` 全部注释。

> **潜在影响**：CentOS 开发环境依赖 `.cshrc` 中的配置，全部注释后可能影响正常使用。建议仅在首次连接时临时处理，完成后恢复原配置。
